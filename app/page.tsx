...
const results:Record<Primary,ResultCopy>={
 ...
 MESSAGE_CONTENT:{title:"המסר שלך לא מספיק מחובר להצעה שאת רוצה לקדם",identified:"יש פעילות ותוכן, אבל לא תמיד ברור איך כל פרסום מחזק את מה שאת רוצה לקדם.",impact:"נוצרת הרבה עשייה בלי ודאות שהיא מייצרת הבנה, עניין או תנועה עסקית.",focus:"לחבר כל תוכן למסר מרכזי אחד ולהצעה אחת.",first:"הגדירי שניים או שלושה מסרים שהקהל חייב להבין לפני שיבחר בהצעה שלך.",notNow:"לא לפרסם יותר רק כדי להיות עקבית ולא להוסיף עוד פורמטים."},
 CONVERSION_PATH:{title:"העניין קיים, אבל הדרך לפנייה נעצרת",identified:"כבר נוצר עניין, אבל המעבר ממנו לפנייה או מהפנייה לשלב הבא אינו חלק מספיק.",impact:"אנשים מתעניינים או אפילו פונים, אבל חלק מהם הולכים לאיבוד בדרך.",focus:"לפשט את הדרך מהעניין לפנייה ולוודא שיש המשך ברור.",first:"בחרי פעולה אחת שאת רוצה שמתעניינת תעשה ובדקי שהיא בולטת ופשוטה.",notNow:"לא להזרים עוד תנועה למסלול לפני שמתקנים אותו."},
...
export default function Home(){
 ...
 return <main dir="rtl">
  ...
  {!started?<section className="hero">
   ...
   <div className="result-hero-card"><div className="result-map-wrap"><div className="result-map-title"><span>מפת האבחון שלך</span><small>שישה תחומים. מיקוד אחד.</small></div><DiagnosticMap primary={calculation.primaryBottleneck} secondary={calculation.secondaryBottleneck}/></div>
   <header className="result-intro"><p>מצאנו את צוואר הבקבוק שלך</p><h1>{result.title}</h1><div className="confidence">רמת ודאות: <strong>{confidenceLabel}</strong></div></header></div>
   <div className="result-content">
    ...
    <section className="summary-box"><small>האבחון במשפט אחד</small><p>{calculation.primaryBottleneck==="INSUFFICIENT_EVIDENCE"?"השלב הנכון כרגע הוא איסוף מידע מסודר, לפני שמקבלים החלטה שיווקית גדולה.":`לפני שמוסיפים עוד שיווק, כדאי לטפל קודם ב${areaLabels[calculation.primaryBottleneck]}.`}</p></section>
    ...
    <section className="action-map"><div className="section-heading"><span>מפת הפעולה שלך</span><small>שלושה צעדים להתחלה ממוקדת</small></div><ol>{[["מה לבדוק",actionMap.check],["מה להחליט",actionMap.decide],["מה ליישם",actionMap.implement]].map(([label,text],index)=><li key={label}><b>{index+1}</b><div><small>{label}</small><p>{text}</p></div></li>)}</ol></section>
