...
const require=createRequire(import.meta.url); const module={exports:{}}; const sandbox={module,exports:module.exports,require,structuredClone,Set,Map,Object,Math}; vm.createContext(sandbox); vm.runInContext(js,sandbox); const {evaluateDiagnostic,CORE_QUESTIONS,buildQuestionPath}=module.exports;
...
test("כל מסלולי הליבה יוצרים 2–4 שאלות הכרעה",()=>{let paths=0;for(const g of CORE_QUESTIONS[0].options)for(const f of CORE_QUESTIONS[1].options)for(const s of CORE_QUESTIONS[2].options)for(const e of CORE_QUESTIONS[3].options)for(const c of CORE_QUESTIONS[4].options){const path=buildQuestionPath(a([["goal",g.id],["focus",f.id],["source",s.id],["exposure",e.id],["capacity",c.id]]));assert.ok(path.length>=2&&path.length<=4);assert.equal(new Set(path.map(q=>q.id)).size,path.length);paths++;}assert.ok(paths>10000);});
